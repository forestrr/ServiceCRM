...
export const QuotationsPage = () => {
    ...
    const handleSaveQuotation = async () => {
        ...
        try {
            ...
            if (editingQuotationId) {
                ...
                // 1. Generate Quotation Number from Profile Settings
                let quotationNumber = `QT-${String(Date.now()).slice(-6)}`; // Fallback
                let nextNum = 1;

                if (profile?.quotation_settings) {
                    const qs = profile.quotation_settings;
                    const pre = qs.prefix || 'QT';
                    const sep = qs.separator || '-';
                    const pad = qs.pad_length || 4;
                    nextNum = qs.next_number || 1;

                    quotationNumber = `${pre}${sep}${String(nextNum).padStart(pad, '0')}`;
                }
                ...
                // 3. Update Profile Settings to increment next_number
                if (profile?.quotation_settings) {
                    const newSettings = { ...profile.quotation_settings, next_number: nextNum + 1 };
                    await supabase
                        .from('profiles')
                        .update({ quotation_settings: newSettings })
                        .eq('id', user?.id);
                }
            }
    ...
    const handleApprove = async (quote: Quotation) => {
        ...
        try {
            ...
            // Invoice will be auto-generated by the workflow engine
            // when the first step is completed (draft) and posted when all steps complete
                                ...
                                {filteredQuotations.map(q => (
                                    <tr key={q.id} className={styles.tr}>
                                        <td className={styles.td}>
                                            <div style={{ fontWeight: 600 }}>{q.quotation_number || `#${q.id.split('-')[0]}`}</div>
                        ...
                        <div className={styles.quotationA4} id="quotation-capture">
                            ...
                            <div className={styles.quotationHeader}>
                                ...
                                <div className={styles.quoteTitleArea}>
                                    ...
                                    <div className={styles.quoteId}>{viewingQuotation.quotation_number || `QT-${viewingQuotation.id.split('-')[0].toUpperCase()}`}</div>
                                </div>
                            </div>
                            ...
                            <div className={styles.metaGrid}>
                                ...
                                <div className={styles.metaDetailsGrid}>
                                    ...
                                    <div className={styles.metaDetailValue}>{viewingQuotation.quotation_number || `QT-${viewingQuotation.id.split('-')[0].toUpperCase()}`}</div>
                                </div>
                            </div>
